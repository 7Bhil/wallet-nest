...
export class CurrencyService implements OnModuleInit {
  ...
  private async fetchRealRates() {
    try {
      console.log(
        '🔄 Actualisation des taux de change (Strategy: Spread 2%)...',
      );
      ...
      if (data && data.rates) {
        ...
        Object.keys(data.rates).forEach((curr) => {
          this.fromBsdRates[curr] =
            data.rates[curr] * (1 - this.SPREAD_COMMISSION);
        });
  ...
  async convertToBSD(amount: number, fromCurrency: string): Promise<number> {
    await this.ensureFreshRates();
    const rate =
      this.toBsdRates[fromCurrency.toUpperCase()] || 1 - this.SPREAD_COMMISSION;
    return amount * rate;
  }
  ...
  async convertFromBSD(bsdAmount: number, toCurrency: string): Promise<number> {
    await this.ensureFreshRates();
    const rate =
      this.fromBsdRates[toCurrency.toUpperCase()] || 1 - this.SPREAD_COMMISSION;
  ...
  async convertFiat(amount: number, from: string, to: string): Promise<number> {
    ...
    if (!rateFrom || !rateTo) {
      throw new Error(
        `Taux de change introuvable pour ${!rateFrom ? from : to}`,
      );
    }
  ...
  async getLatestRate(currency: string): Promise<number> {
    await this.ensureFreshRates();
    return (
      this.toBsdRates[currency.toUpperCase()] || 1 - this.SPREAD_COMMISSION
    );
  }

  /**
   * Convertit un montant exact d'une devise à une autre (utilisé pour les MAJ de profil et création de compte)
  ...
  async convertExact(
    ...
    const rateFrom = this.rawRates[fromUpper];
    const rateTo = this.rawRates[toUpper];

    if (!rateFrom || !rateTo) {
      // Fallback: Tentative de calcul via les taux codés en dur si l'API a échoué
      console.warn(
        `[CurrencyService] Taux manquant pour ${!rateFrom ? fromUpper : toUpper} dans rawRates. Utilisation des fallbacks.`,
      );

      // On sait que toBSD[X] = (1/rateX) * (1-0.02)
      // Donc rateX = (1 - 0.02) / toBSD[X]
      const fallbackRateFrom =
        (1 - this.SPREAD_COMMISSION) / (this.toBsdRates[fromUpper] || 1);
      const fallbackRateTo =
        (1 - this.SPREAD_COMMISSION) / (this.toBsdRates[toUpper] || 1);

      const amountInUsd = amount / fallbackRateFrom;
      return amountInUsd * fallbackRateTo;
    }
  ...
  private rawRates: { [key: string]: number } = {
    USD: 1,
    EUR: 0.92,
    XOF: 610,
    GBP: 0.78,
