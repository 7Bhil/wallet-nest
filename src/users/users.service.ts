...
export class UsersService {
  ...
  async updateBalance(id: string, amount: number, session?: any): Promise<User | null> {
  ...
  async deductBalanceSafe(id: string, amount: number, session?: any): Promise<User | null> {
  ...
  async updateProfile(id: string, data: Partial<User>): Promise<User | null> {
    ...
    if (data.currency && data.currency !== user.currency) {
      ...
      const exactConverted = await this.currencyService.convertExact(user.balance, oldCurrency, newCurrency);
      ...
      for (const card of cards) {
        const newCardBalance = await this.currencyService.convertExact(card.cardBalance, oldCurrency, newCurrency);
        const newLimitValue = await this.currencyService.convertExact(card.limitValue, oldCurrency, newCurrency);
      ...
      console.log(`[FX CRITICAL FIX] Converted User ${id} and all cards from ${oldCurrency} to ${newCurrency}`);
    }
